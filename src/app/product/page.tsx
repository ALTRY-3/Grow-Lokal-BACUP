...
export default function ProductPage() {
    ...
    try {
      ...
      if (data.success && data.data?.isSeller && data.status === "approved") {
        ...
        if (data.status === "pending") {
          setError(
            "Your seller application is pending approval. Please wait for admin review."
          );
          ...
          setError(
            "You need to be an approved seller to manage products. Please apply to become a seller first."
          );
        }
      }
    } catch (error) {
      ...
      if (data.success) {
        alert(
          `User: ${data.userInfo.email}\nSeller Status: ${
            data.userInfo.sellerProfile?.applicationStatus || "Not a seller"
    ...
    try {
      ...
      if (data.success) {
        ...
        if (response.status === 403) {
          setError(
            "You need to be an approved seller to view products. Please apply to become a seller first."
          );
  ...
  const filteredProducts = products.filter((p) => {
    ...
    if (activeSection === "live")
      matchesSection = p.isActive && p.isAvailable && p.stock > 0;
    else if (activeSection === "soldout")
      matchesSection = p.isActive && p.isAvailable && p.stock === 0;
    else if (activeSection === "draft") matchesSection = !p.isActive;
    else if (activeSection === "inactive")
      matchesSection = p.isActive && !p.isAvailable;
  ...
  const sectionCounts = {
    live: products.filter((p) => p.isActive && p.isAvailable && p.stock > 0)
      .length,
    soldout: products.filter(
      (p) => p.isActive && p.isAvailable && p.stock === 0
    ).length,
      ...
      <>
        ...
        <div className="product-page-wrapper">
          ...
          >
            <FaSpinner
              className="fa-spin"
              style={{ fontSize: "3rem", color: "#AF7928" }}
            />
              ...
              <div className="product-card-enhanced" key={product._id}>
                ...
                <div className="product-card-image-container">
                  ...
                  <div className="product-status-badge-container">
                    ...
                    {product.isActive && !product.isAvailable && (
                      <span className="product-status-badge inactive">
                        Inactive
                      </span>
                    )}
                    {product.isActive &&
                      product.isAvailable &&
                      product.stock === 0 && (
                        <span className="product-status-badge soldout">
                          Sold Out
                        </span>
                      )}
                    {product.isActive &&
                ...
                <div className="product-card-info-enhanced">
                  ...
                  <div className="product-card-header">
                    <h3 className="product-card-name-enhanced">
                      {product.name}
                    </h3>
                  ...
                  <div className="product-card-price-stock-row">
                    ...
                    <div className="product-card-stock-container">
                      ...
                      <span
                        className={`product-card-stock-value ${
                          product.stock === 0
                            ? "out-of-stock"
                            : product.stock <= 5
                            ? "low-stock"
                  ...
                  <div className="product-card-actions">
                    <button
                      onClick={() =>
                        router.push(`/add-product?edit=${product._id}`)
                      }
